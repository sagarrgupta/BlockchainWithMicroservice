...
def mine_and_broadcast_block(blockTransactions):
    ...
    for peer in master_peers:
        try:
            ...
            resp = requests.post(f"http://{host_port}/receive_block", json={'block': new_block}, headers=headers, timeout=2)
...
def update_resource_allocation(city_id, risk_level):
    ...
    for master in master_peers:
        try:
            ...
            resp = requests.post(f"http://{host_port}/receive_block", json={'block': block}, headers=headers, timeout=3)
            if resp.status_code == 200:
