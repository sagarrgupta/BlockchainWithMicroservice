...
URL="${URL:-http://localhost:5003/update_resource/1/medium}"
SLEEP="${SLEEP:-0.5}"
...
# ---- state: last observed counts (init to -1 so first refresh sets them) ----
LAST_TOTAL_PODS=-1
LAST_MASTER_PODS=-1
...
summarize() {
  ...
  # use the *current* counts we just computed
...
on_exit() {
  echo
  # final snapshot with current counts (but only if we have data)
  refresh_counts
...
# ---- event gate: only snapshot if counts actually changed ----
on_possible_event() {
  local source="${1:-K8s}"
  refresh_counts
